...
//        types = "24";  // 影视金曲
    ...
    private void setInfo(final ViewHolder holder,final int position) {
        if (list.size() <= 1){
            ...
            HttpClient.getOnlineMusicList("24", 30, 0, new GetCallBack<OnLineMusicList>() {
                @Override
    ...
    private void downloadMusic(Music music){
        HttpClient.download(music, new DownloadCallBack() {
            @Override
            public void onMusicSuccess() throws IOException {
                L.i("更新媒体库完成，开始扫描");
                scanMusic();
            }
     ...
     * 扫描媒体库进行更新音乐
     */
    private void scanMusic() throws IOException {
